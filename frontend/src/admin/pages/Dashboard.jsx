...
export default function Dashboard() {
  ...
  const winRate = (() => {
    const completed = data?.fixtures_completed ?? 0
    if (!completed) return null
    // We can't calculate wins from dashboard alone, skip
    return null
  })()
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Primary stats */}
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Club Overview</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard label="Gallery Photos"     value={data?.photos}              icon={Images} />
          <StatCard
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          <QuickLink to="/admin/players"   icon={Users}       label="Manage Players & Staff" />
          <QuickLink to="/admin/fixtures"  icon={CalendarDays} label="Add / Edit Fixtures" />
          <QuickLink to="/admin/news"      icon={Newspaper}   label="Write New Article" />
          <QuickLink to="/admin/gallery"   icon={Images}      label="Upload Photos" />
          <QuickLink to="/admin/enquiries" icon={Mail}        label="View Enquiries" badge={data?.enquiries_unread} />
          <QuickLink to="/admin/sponsors"  icon={Handshake}   label="Manage Sponsors" />
        </div>
      </div>
    </div>
  )
